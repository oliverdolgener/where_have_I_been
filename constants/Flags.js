import Afghanistan from '../assets/flags/Afghanistan.png';
import Albania from '../assets/flags/Albania.png';
import Algeria from '../assets/flags/Algeria.png';
import Andorra from '../assets/flags/Andorra.png';
import Angola from '../assets/flags/Angola.png';
import Antigua from '../assets/flags/Antigua-and-Barbuda.png';
import Argentina from '../assets/flags/Argentina.png';
import Armenia from '../assets/flags/Armenia.png';
import Australia from '../assets/flags/Australia.png';
import Austria from '../assets/flags/Austria.png';
import Azerbaijan from '../assets/flags/Azerbaijan.png';
import Bahamas from '../assets/flags/Bahamas.png';
import Bahrain from '../assets/flags/Bahrain.png';
import Bangladesh from '../assets/flags/Bangladesh.png';
import Barbados from '../assets/flags/Barbados.png';
import Belarus from '../assets/flags/Belarus.png';
import Belgium from '../assets/flags/Belgium.png';
import Belize from '../assets/flags/Belize.png';
import Benin from '../assets/flags/Benin.png';
import Bhutan from '../assets/flags/Bhutan.png';
import Bolivia from '../assets/flags/Bolivia.png';
import Bosnia from '../assets/flags/Bosnia-and-Herzegovina.png';
import Botswana from '../assets/flags/Botswana.png';
import Brazil from '../assets/flags/Brazil.png';
import Brunei from '../assets/flags/Brunei.png';
import Bulgaria from '../assets/flags/Bulgaria.png';
import BurkinaFaso from '../assets/flags/Burkina-Faso.png';
import Burma from '../assets/flags/Myanmar.png';
import Burundi from '../assets/flags/Burundi.png';
import Cambodia from '../assets/flags/Cambodia.png';
import Cameroon from '../assets/flags/Cameroon.png';
import Canada from '../assets/flags/Canada.png';
import CapeVerde from '../assets/flags/Cape-Verde.png';
import CentralAfricanRepublic from '../assets/flags/Central-African-Republic.png';
import Chad from '../assets/flags/Chad.png';
import Chile from '../assets/flags/Chile.png';
import China from '../assets/flags/China.png';
import Colombia from '../assets/flags/Colombia.png';
import Comoros from '../assets/flags/Comoros.png';
import CostaRica from '../assets/flags/Costa-Rica.png';
import IvoryCoast from '../assets/flags/Cote-dIvoire.png';
import Croatia from '../assets/flags/Croatia.png';
import Cuba from '../assets/flags/Cuba.png';
import Cyprus from '../assets/flags/Cyprus.png';
import CzechRepublic from '../assets/flags/Czech-Republic.png';
import DemocraticRepublicOfTheCongo from '../assets/flags/Democratic-Republic-of-the-Congo.png';
import Denmark from '../assets/flags/Denmark.png';
import Djibouti from '../assets/flags/Djibouti.png';
import Dominica from '../assets/flags/Dominica.png';
import DominicanRepublic from '../assets/flags/Dominican-Republic.png';
import EastTimor from '../assets/flags/East-Timor.png';
import Ecuador from '../assets/flags/Ecuador.png';
import Egypt from '../assets/flags/Egypt.png';
import ElSalvador from '../assets/flags/El-Salvador.png';
import EquatorialGuinea from '../assets/flags/Equatorial-Guinea.png';
import Eritrea from '../assets/flags/Eritrea.png';
import Estonia from '../assets/flags/Estonia.png';
import Ethiopia from '../assets/flags/Ethiopia.png';
import FederatedStatesOfMicronesia from '../assets/flags/Micronesia.png';
import Fiji from '../assets/flags/Fiji.png';
import Finland from '../assets/flags/Finland.png';
import France from '../assets/flags/France.png';
import Gabon from '../assets/flags/Gabon.png';
import Gambia from '../assets/flags/Gambia.png';
import Georgia from '../assets/flags/Georgia.png';
import Ghana from '../assets/flags/Ghana.png';
import Germany from '../assets/flags/Germany.png';
import Greece from '../assets/flags/Greece.png';
import Grenada from '../assets/flags/Grenada.png';
import Guatemala from '../assets/flags/Guatemala.png';
import Guinea from '../assets/flags/Guinea.png';
import GuineaBissau from '../assets/flags/Guinea-Bissau.png';
import Guyana from '../assets/flags/Guyana.png';
import Haiti from '../assets/flags/Haiti.png';
import Honduras from '../assets/flags/Honduras.png';
import Hungary from '../assets/flags/Hungary.png';
import Iceland from '../assets/flags/Iceland.png';
import India from '../assets/flags/India.png';
import Indonesia from '../assets/flags/Indonesia.png';
import Iran from '../assets/flags/Iran.png';
import Iraq from '../assets/flags/Iraq.png';
import Ireland from '../assets/flags/Ireland.png';
import Israel from '../assets/flags/Israel.png';
import Italy from '../assets/flags/Italy.png';
import Jamaica from '../assets/flags/Jamaica.png';
import Japan from '../assets/flags/Japan.png';
import Jordan from '../assets/flags/Jordan.png';
import Kazakhstan from '../assets/flags/Kazakhstan.png';
import Kenya from '../assets/flags/Kenya.png';
import Kiribati from '../assets/flags/Kiribati.png';
import Kuwait from '../assets/flags/Kuwait.png';
import Kyrgyzstan from '../assets/flags/Kyrgyzstan.png';
import Laos from '../assets/flags/Laos.png';
import Latvia from '../assets/flags/Latvia.png';
import Lebanon from '../assets/flags/Lebanon.png';
import Lesotho from '../assets/flags/Lesotho.png';
import Liberia from '../assets/flags/Liberia.png';
import Libya from '../assets/flags/Libya.png';
import Liechtenstein from '../assets/flags/Liechtenstein.png';
import Lithuania from '../assets/flags/Lithuania.png';
import Luxembourg from '../assets/flags/Luxembourg.png';
import Macedonia from '../assets/flags/Macedonia.png';
import Madagascar from '../assets/flags/Madagascar.png';
import Malawi from '../assets/flags/Malawi.png';
import Malaysia from '../assets/flags/Malaysia.png';
import Maldives from '../assets/flags/Maldives.png';
import Mali from '../assets/flags/Mali.png';
import Malta from '../assets/flags/Malta.png';
import MarshallIslands from '../assets/flags/Marshall-Islands.png';
import Mauritania from '../assets/flags/Mauritania.png';
import Mauritius from '../assets/flags/Mauritius.png';
import Mexico from '../assets/flags/Mexico.png';
import Moldova from '../assets/flags/Moldova.png';
import Monaco from '../assets/flags/Monaco.png';
import Mongolia from '../assets/flags/Mongolia.png';
import Montenegro from '../assets/flags/Montenegro.png';
import Morocco from '../assets/flags/Morocco.png';
import Mozambique from '../assets/flags/Mozambique.png';
import Namibia from '../assets/flags/Namibia.png';
import Nauru from '../assets/flags/Nauru.png';
import Nepal from '../assets/flags/Nepal.png';
import Netherlands from '../assets/flags/Netherlands.png';
import NewZealand from '../assets/flags/New-Zealand.png';
import Nicaragua from '../assets/flags/Nicaragua.png';
import Niger from '../assets/flags/Niger.png';
import Nigeria from '../assets/flags/Nigeria.png';
import NorthKorea from '../assets/flags/North-Korea.png';
import Norway from '../assets/flags/Norway.png';
import Oman from '../assets/flags/Oman.png';
import Pakistan from '../assets/flags/Pakistan.png';
import Palau from '../assets/flags/Palau.png';
import Palestine from '../assets/flags/Palestine.png';
import Panama from '../assets/flags/Panama.png';
import PapuaNewGuinea from '../assets/flags/Papua-New-Guinea.png';
import Paraguay from '../assets/flags/Paraguay.png';
import Peru from '../assets/flags/Peru.png';
import Philippines from '../assets/flags/Philippines.png';
import Poland from '../assets/flags/Poland.png';
import Portugal from '../assets/flags/Portugal.png';
import Qatar from '../assets/flags/Qatar.png';
import RepublicOfTheCongo from '../assets/flags/Republic-of-the-Congo.png';
import Romania from '../assets/flags/Romania.png';
import Russia from '../assets/flags/Russia.png';
import Rwanda from '../assets/flags/Rwanda.png';
import SaintKittsAndNevis from '../assets/flags/Saint-Kitts-and-Nevis.png';
import SaintLucia from '../assets/flags/Saint-Lucia.png';
import SaintVincentAndTheGrenadines from '../assets/flags/Saint-Vincent-and-the-Grenadines.png';
import Samoa from '../assets/flags/Samoa.png';
import SanMarino from '../assets/flags/San-Marino.png';
import SaoTomeAndPrincipe from '../assets/flags/Sao-Tome-and-Principe.png';
import SaudiArabia from '../assets/flags/Saudi-Arabia.png';
import Senegal from '../assets/flags/Senegal.png';
import Serbia from '../assets/flags/Serbia.png';
import Seychelles from '../assets/flags/Seychelles.png';
import SierraLeone from '../assets/flags/Sierra-Leone.png';
import Singapore from '../assets/flags/Singapore.png';
import Slovakia from '../assets/flags/Slovakia.png';
import Slovenia from '../assets/flags/Slovenia.png';
import SolomonIslands from '../assets/flags/Solomon-Islands.png';
import Somalia from '../assets/flags/Somalia.png';
import SouthAfrica from '../assets/flags/South-Africa.png';
import SouthKorea from '../assets/flags/South-Korea.png';
import SouthSudan from '../assets/flags/South-Sudan.png';
import Spain from '../assets/flags/Spain.png';
import SriLanka from '../assets/flags/Sri-Lanka.png';
import Sudan from '../assets/flags/Sudan.png';
import Suriname from '../assets/flags/Suriname.png';
import Swaziland from '../assets/flags/Swaziland.png';
import Sweden from '../assets/flags/Sweden.png';
import Switzerland from '../assets/flags/Switzerland.png';
import Syria from '../assets/flags/Syria.png';
import Tajikistan from '../assets/flags/Tajikistan.png';
import Tanzania from '../assets/flags/Tanzania.png';
import Thailand from '../assets/flags/Thailand.png';
import Togo from '../assets/flags/Togo.png';
import Tonga from '../assets/flags/Tonga.png';
import TrinidadAndTobago from '../assets/flags/Trinidad-and-Tobago.png';
import Tunisia from '../assets/flags/Tunisia.png';
import Turkey from '../assets/flags/Turkey.png';
import Turkmenistan from '../assets/flags/Turkmenistan.png';
import Tuvalu from '../assets/flags/Tuvalu.png';
import Uganda from '../assets/flags/Uganda.png';
import Ukraine from '../assets/flags/Ukraine.png';
import UnitedArabEmirates from '../assets/flags/United-Arab-Emirates.png';
import UnitedKingdom from '../assets/flags/United-Kingdom.png';
import UnitedStates from '../assets/flags/United-States.png';
import Uruguay from '../assets/flags/Uruguay.png';
import Uzbekistan from '../assets/flags/Uzbekistan.png';
import Vanuatu from '../assets/flags/Vanuatu.png';
import VaticanCity from '../assets/flags/Vatican-City.png';
import Venezuela from '../assets/flags/Venezuela.png';
import Vietnam from '../assets/flags/Vietnam.png';
import Yemen from '../assets/flags/Yemen.png';
import Zambia from '../assets/flags/Zambia.png';
import Zimbabwe from '../assets/flags/Zimbabwe.png';

export default [
  { id: 1, flag: Afghanistan },
  { id: 2, flag: Albania },
  { id: 3, flag: Algeria },
  { id: 4, flag: Andorra },
  { id: 5, flag: Angola },
  { id: 6, flag: Antigua },
  { id: 7, flag: Argentina },
  { id: 8, flag: Armenia },
  { id: 9, flag: Australia },
  { id: 10, flag: Austria },
  { id: 11, flag: Azerbaijan },
  { id: 12, flag: Bahamas },
  { id: 13, flag: Bahrain },
  { id: 14, flag: Bangladesh },
  { id: 15, flag: Barbados },
  { id: 16, flag: Belarus },
  { id: 17, flag: Belgium },
  { id: 18, flag: Belize },
  { id: 19, flag: Benin },
  { id: 20, flag: Bhutan },
  { id: 21, flag: Bolivia },
  { id: 22, flag: Bosnia },
  { id: 23, flag: Botswana },
  { id: 24, flag: Brazil },
  { id: 25, flag: Brunei },
  { id: 26, flag: Bulgaria },
  { id: 27, flag: BurkinaFaso },
  { id: 28, flag: Burma },
  { id: 29, flag: Burundi },
  { id: 30, flag: Cambodia },
  { id: 31, flag: Cameroon },
  { id: 32, flag: Canada },
  { id: 33, flag: CapeVerde },
  { id: 34, flag: CentralAfricanRepublic },
  { id: 35, flag: Chad },
  { id: 36, flag: Chile },
  { id: 37, flag: China },
  { id: 38, flag: Colombia },
  { id: 39, flag: Comoros },
  { id: 40, flag: CostaRica },
  { id: 41, flag: IvoryCoast },
  { id: 42, flag: Croatia },
  { id: 43, flag: Cuba },
  { id: 44, flag: Cyprus },
  { id: 45, flag: CzechRepublic },
  { id: 46, flag: DemocraticRepublicOfTheCongo },
  { id: 47, flag: Denmark },
  { id: 48, flag: Djibouti },
  { id: 49, flag: Dominica },
  { id: 50, flag: DominicanRepublic },
  { id: 51, flag: EastTimor },
  { id: 52, flag: Ecuador },
  { id: 53, flag: Egypt },
  { id: 54, flag: ElSalvador },
  { id: 55, flag: EquatorialGuinea },
  { id: 56, flag: Eritrea },
  { id: 57, flag: Estonia },
  { id: 58, flag: Ethiopia },
  { id: 59, flag: FederatedStatesOfMicronesia },
  { id: 60, flag: Fiji },
  { id: 61, flag: Finland },
  { id: 62, flag: France },
  { id: 63, flag: Gabon },
  { id: 64, flag: Gambia },
  { id: 65, flag: Georgia },
  { id: 66, flag: Germany },
  { id: 67, flag: Ghana },
  { id: 68, flag: Greece },
  { id: 69, flag: Grenada },
  { id: 70, flag: Guatemala },
  { id: 71, flag: Guinea },
  { id: 72, flag: GuineaBissau },
  { id: 73, flag: Guyana },
  { id: 74, flag: Haiti },
  { id: 75, flag: Honduras },
  { id: 76, flag: Hungary },
  { id: 77, flag: Iceland },
  { id: 78, flag: India },
  { id: 79, flag: Indonesia },
  { id: 80, flag: Iran },
  { id: 81, flag: Iraq },
  { id: 82, flag: Ireland },
  { id: 83, flag: Israel },
  { id: 84, flag: Italy },
  { id: 85, flag: Jamaica },
  { id: 86, flag: Japan },
  { id: 87, flag: Jordan },
  { id: 88, flag: Kazakhstan },
  { id: 89, flag: Kenya },
  { id: 90, flag: Kiribati },
  { id: 91, flag: Kuwait },
  { id: 92, flag: Kyrgyzstan },
  { id: 93, flag: Laos },
  { id: 94, flag: Latvia },
  { id: 95, flag: Lebanon },
  { id: 96, flag: Lesotho },
  { id: 97, flag: Liberia },
  { id: 98, flag: Libya },
  { id: 99, flag: Liechtenstein },
  { id: 100, flag: Lithuania },
  { id: 101, flag: Luxembourg },
  { id: 102, flag: Macedonia },
  { id: 103, flag: Madagascar },
  { id: 104, flag: Malawi },
  { id: 105, flag: Malaysia },
  { id: 106, flag: Maldives },
  { id: 107, flag: Mali },
  { id: 108, flag: Malta },
  { id: 109, flag: MarshallIslands },
  { id: 110, flag: Mauritania },
  { id: 111, flag: Mauritius },
  { id: 112, flag: Mexico },
  { id: 113, flag: Moldova },
  { id: 114, flag: Monaco },
  { id: 115, flag: Mongolia },
  { id: 116, flag: Montenegro },
  { id: 117, flag: Morocco },
  { id: 118, flag: Mozambique },
  { id: 119, flag: Namibia },
  { id: 120, flag: Nauru },
  { id: 121, flag: Nepal },
  { id: 122, flag: Netherlands },
  { id: 123, flag: NewZealand },
  { id: 124, flag: Nicaragua },
  { id: 125, flag: Niger },
  { id: 126, flag: Nigeria },
  { id: 127, flag: NorthKorea },
  { id: 128, flag: Norway },
  { id: 129, flag: Oman },
  { id: 130, flag: Pakistan },
  { id: 131, flag: Palau },
  { id: 132, flag: Palestine },
  { id: 133, flag: Panama },
  { id: 134, flag: PapuaNewGuinea },
  { id: 135, flag: Paraguay },
  { id: 136, flag: Peru },
  { id: 137, flag: Philippines },
  { id: 138, flag: Poland },
  { id: 139, flag: Portugal },
  { id: 140, flag: Qatar },
  { id: 141, flag: RepublicOfTheCongo },
  { id: 142, flag: Romania },
  { id: 143, flag: Russia },
  { id: 144, flag: Rwanda },
  { id: 145, flag: SaintKittsAndNevis },
  { id: 146, flag: SaintLucia },
  { id: 147, flag: SaintVincentAndTheGrenadines },
  { id: 148, flag: Samoa },
  { id: 149, flag: SanMarino },
  { id: 150, flag: SaoTomeAndPrincipe },
  { id: 151, flag: SaudiArabia },
  { id: 152, flag: Senegal },
  { id: 153, flag: Serbia },
  { id: 154, flag: Seychelles },
  { id: 155, flag: SierraLeone },
  { id: 156, flag: Singapore },
  { id: 157, flag: Slovakia },
  { id: 158, flag: Slovenia },
  { id: 159, flag: SolomonIslands },
  { id: 160, flag: Somalia },
  { id: 161, flag: SouthAfrica },
  { id: 162, flag: SouthKorea },
  { id: 163, flag: SouthSudan },
  { id: 164, flag: Spain },
  { id: 165, flag: SriLanka },
  { id: 166, flag: Sudan },
  { id: 167, flag: Suriname },
  { id: 168, flag: Swaziland },
  { id: 169, flag: Sweden },
  { id: 170, flag: Switzerland },
  { id: 171, flag: Syria },
  { id: 172, flag: Tajikistan },
  { id: 173, flag: Tanzania },
  { id: 174, flag: Thailand },
  { id: 175, flag: Togo },
  { id: 176, flag: Tonga },
  { id: 177, flag: TrinidadAndTobago },
  { id: 178, flag: Tunisia },
  { id: 179, flag: Turkey },
  { id: 180, flag: Turkmenistan },
  { id: 181, flag: Tuvalu },
  { id: 182, flag: Uganda },
  { id: 183, flag: Ukraine },
  { id: 184, flag: UnitedArabEmirates },
  { id: 185, flag: UnitedKingdom },
  { id: 186, flag: UnitedStates },
  { id: 187, flag: Uruguay },
  { id: 188, flag: Uzbekistan },
  { id: 189, flag: Vanuatu },
  { id: 190, flag: VaticanCity },
  { id: 191, flag: Venezuela },
  { id: 192, flag: Vietnam },
  { id: 193, flag: Yemen },
  { id: 194, flag: Zambia },
  { id: 195, flag: Zimbabwe },
];
